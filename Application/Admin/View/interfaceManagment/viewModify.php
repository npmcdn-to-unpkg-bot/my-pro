	...
	<div class="row-fluid">
		<div class="span12">
			<div class="widget-box">
				...
				<form class="form-horizontal" method="post" action="__ROOT__/{$Think.CONTROLLER_NAME}/modify" name="interface" id="interface_validate" novalidate="novalidate">
					<div class="widget-content nopadding tab-content">
						<div id="basicsInfoTab" class="tab-pane active">
							<div class="control-group">
								...
								<div class="controls">
									<input type="text" class="span5" name="name" id="name"
										placeholder="请输入接口名称(必填)"  value="{$data['name']}"/>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<select name="category" id="category" class="span5" >
										...
										<option value="{$top.id}/{$top.name}" <if condition="$top.id eq $data['category_id']">selected="selected"</if>>{$top.name}</option>
											<volist name="top.children" id='chls'>
												<option value="{$chls.id}/{$chls.name}" <if condition="$chls.id eq $data['category_id']">selected="selected"</if>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;----{$chls.name}</option>
											</volist> 
										</volist>
									</select>
								</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<label class="fl "> <input type="radio" name="method" value="00A" <if condition="$data['int_method'] eq '00A'">checked="checked"</if>/> GET方法
									</label> <label class="fl"> <input type="radio" name="method" <if condition="$data['int_method'] eq '00B'">checked="checked"</if>value="00B" /> POST方法
									</label>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<input type="text" class="span5" name="url" id="url" placeholder="请输入生产环境接口地址（完整带Http地址）" value="{$data['url']}"/>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<input type="text" class="span5" name="testUrl" id="testUrl" placeholder="请输入生产环境接口地址（完整带Http地址）"  value="{$data['test_url']}"/>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<script id="demoIn" type="text/plain" style="width: 95%; height: 150px;">{$data['demo_in']}</script>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<input type="text" name="sort" id="sort" class="span5"  placeholder="请输入接口展示排序（越大越前，必填）" value="{$data['sort']}"/>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<textarea name="discription" id="discription" class="span11"  placeholder="请输入接口描述">{$data['discription']}</textarea>
								</div>
							</div>
							...
							<div class="control-group">
								...
								<div class="controls">
									<textarea name="remark" id="remark" class="span11" placeholder="请输入接口备注">{$data['remark']}</textarea>
								</div>
							</div>
						</div>
						...
						<div class="form-actions">
							...
							<button type="button" class="btn btn-danger fa fa-reply"
								onclick="javascript:window.location='__ROOT__/{$Think.CONTROLLER_NAME}/viewList/m_id/{:I('m_id')}/p_id/{:I('p_id')}'">
								返回</button>
						</div>
					</div>
