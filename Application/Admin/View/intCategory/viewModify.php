		...
		<div class="span12">
			<div class="widget-box">
				...
				<div class="widget-content nopadding">
					<form class="form-horizontal" method="post" action="__ROOT__/{$Think.CONTROLLER_NAME}/modify" name="menu" id="category_validate" novalidate="novalidate">
						<div class="control-group">
							<label class="control-label">接口分类名称</label>
							<div class="controls">
								<input type="text" class="span5" name="category_name" id="category_name" placeholder="请输入接口分类名称(必填)" value="{$data['name']}" />
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls">
								<select  name="pid" id="pid" class="span5">
									...
									<option value="{$vo.id}/{$vo.name}"  <if condition="$vo.id eq $data['pid']">selected="selected"</if> >{$vo.name}</option>
									</volist>
								</select>
							</div>
						</div>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" name="sort" id="sort" class="span5" placeholder="请输入菜单排序（越大越前，必填）" value="{$data['sort']}"/>
							</div>
						</div>
						<div class="control-group">
							<label class="control-label">分类描述</label>
							<div class="controls">
								<textarea  name="discription" id="discription" class="span11" placeholder="请输入分类描述" >{$data['discription']}</textarea>
							</div>
						</div>
