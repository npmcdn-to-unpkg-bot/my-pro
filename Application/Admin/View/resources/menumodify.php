			...
			<div class="widget-box">
				...
				<div class="widget-content nopadding">
					<form class="form-horizontal" method="post"
						action="__ROOT__/resources/menumodify" name="menu"
						id="menu_validate" novalidate="novalidate">
						<div class="control-group">
							<label class="control-label">菜单名称</label>
							<div class="controls">
								<input type="text" name="menu_name" id="menu_name"
									style="width: 300px;" value="{$data[0]['menu_name']}" />
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls">
								<select style="width: 315px;" name="pid" id="pid">
									<option value="none"<if condition="$data[0]['pid'] eq null">selected="selected"</if>>我就是父菜单
									</option>
									<volist name="mainMenu" id="vo">
									<option value="{$vo.id}/{$vo.menu_name}"<if
											condition="$vo.id eq $data[0]['pid']">selected="selected"</if>>{$vo.menu_name}
									</option>
									</volist>
								</select>
							</div>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="hidden" name="icon" id="icon" value="{$data[0]['menu_icon']}" /> <a href="#myIcons" data-toggle="modal"><button class="btn"><i class="fa {$data[0]['menu_icon']}" style="margin-right: 20px;" id="selectedI"></i>点击选择</button></a>
							</div>
						</div>
						...
						<div class="form-actions">
							...
							<button type="button" class="btn btn-danger fa fa-reply" onclick="javascript:window.location='__ROOT__/{$Think.CONTROLLER_NAME}/menulist/m_id/{:I('m_id')}/p_id/{:I('p_id')}'"> 返回</button>
						</div>
					</form>
