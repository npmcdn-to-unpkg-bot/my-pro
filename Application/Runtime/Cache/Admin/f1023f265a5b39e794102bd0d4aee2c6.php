		...
		<div class="span12">
			...
			<div class="widget-box">
				...
				<a href="#mySearch" data-toggle="modal"><button class="btn fa fa-filter font-size-15"> 高级检索</button></a>
					...
					<a href="/myframework/<?php echo (CONTROLLER_NAME); ?>/menuadd/m_id/<?php echo I('m_id');?>/p_id/<?php echo I('p_id');?>" ><button class="btn btn-success  fr fa fa-plus"> 新增菜单</button></a>
				</div>
					...
					<table class="table table-bordered  table-striped with-check">
						...
						<tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td <?php if($vo["pname"] == null): ?>class="font_red"<?php endif; ?>>
								<?php if($vo["pname"] != null): echo ($vo["pname"]); else: ?>一级菜单<?php endif; ?>
								</td>
								<td><?php echo ($vo["uri"]); ?></td>
								<td class="center">
									<div id="sort_btu_<?php echo ($vo["id"]); ?>">
										<button class="btn btn-mini" style="width:100px;" id="btu_<?php echo ($vo["id"]); ?>" onclick="javascript:showInput('<?php echo ($vo["id"]); ?>');"><?php echo ($vo["sort"]); ?></button>
									</div>
									<div class="controls" id="intpu_<?php echo ($vo["id"]); ?>" style="display: none;">
										<input type="text" placeholder="<?php echo ($vo["sort"]); ?>" data-title="请输入数字" class="span11 tip" data-original-title="" style="width: 100px;" id="content_<?php echo ($vo["id"]); ?>" onblur="javascript:lostfocus();"/>
										<button class="btn btn-mini" onclick="javascript:updateSort('<?php echo ($vo["id"]); ?>');">确定</button>
									</div>
								</td>
								<td><i class="fa <?php echo ($vo["menu_icon"]); ?>" style="margin-right: 10px;"></i><?php echo ($vo["menu_icon"]); ?></td>
								...
								<td>
									<div class="btn-group">
										...
										<ul class="dropdown-menu">
											<li><a href="/myframework/resources/menumodify/m_id/<?php echo I('m_id');?>/p_id/<?php echo I('p_id');?>/id/<?php echo ($vo["id"]); ?>" class="fa fa-edit"> 修改</a></li>
											<li class="divider"></li>
	            ...
	            <div id="mySearch" class="modal hide">
	              ...
	              <div class="modal-body">
			          <form action="/myframework/<?php echo (CONTROLLER_NAME); ?>/menulist/" method="get" id="searchFrom" class="form-horizontal">
			            <div class="control-group">
...
<script type="text/javascript">
		var deleteUrl = "/myframework/<?php echo (CONTROLLER_NAME); ?>/menuDelete/";
		...
		function updateSort(id){
			...
			if(!isNaN(newSort)){
				...
				$.ajax({
					type: "POST",
					dataType:"json",
					url: "/myframework/<?php echo (CONTROLLER_NAME); ?>/modifySort/",
					data: param,
